...
CScanTask::CScanTask(HANDLE hProcess, int iProcessID)
	: m_hProcess(hProcess), m_bCompleted(false)
{
}
...
bool CScanTask::EnumerateRegions(std::vector<CScanTask::SMemoryRegion*>* vMemoryRegions)
{
	...
	Logger::Instance().Log(LogLevel::Info, "Enumerating memory regions from 0x%llx to 0x%llx",
		reinterpret_cast<DWORD64>(sysInfo.lpMinimumApplicationAddress),
		...
		lpCurrentAddress = static_cast<LPBYTE>(MemoryRegion.BaseAddress) + MemoryRegion.RegionSize)
	{
		...
		if (!NT_SUCCESS(status = SysNtQueryVirtualMemory(m_hProcess, lpCurrentAddress, MemoryBasicInformation, &MemoryRegion,
			sizeof(MemoryRegion), &returnLength)))
		{
			Logger::Instance().Log(LogLevel::Warning, "Failed to query memory region at address 0x%llx, error: %d", reinterpret_cast<DWORD64>(lpCurrentAddress), status);
			continue;
		}
		...
		vMemoryRegions->push_back(new SMemoryRegion{
			reinterpret_cast<DWORD64>(MemoryRegion.BaseAddress),
...
void CScanTask::ScanRegion(SMemoryRegion* pRegion)
{
	...
	if (NT_SUCCESS(SysNtReadVirtualMemory(m_hProcess, reinterpret_cast<PVOID>(pRegion->dwBaseAddress), buffer.data(), pRegion->Size, &bytesRead)))
	{
		...
		{
			Logger::Instance().Log(LogLevel::Warning, "IAT thunk found at address: 0x%llx in region starting at 0x%llx", dwThunkAddress, pRegion->dwBaseAddress);
		}
	}
}
